...
# IMPORTANT: include localhost for live_server (Playwright runs on http://localhost:<port>)
ALLOWED_HOSTS = [
    "localhost",
    "127.0.0.1",
    "[::1]",
...
# This makes tests almost 2x faster; we don't need strong security and DEBUG during tests
PASSWORD_HASHERS = [
...
# ---------------------------------------------------------------------
# FIX for Playwright e2e tests:
# live_server is http://localhost:<port>, but base.py defaults force HTTPS and Secure cookies.
# That makes session cookie not sent -> redirects to /login.
# ---------------------------------------------------------------------
SECURE_SSL_REDIRECT = False
SESSION_COOKIE_SECURE = False
CSRF_COOKIE_SECURE = False

# Make cookies host-only for localhost (not tied to .compscicenter.ru)
SESSION_COOKIE_DOMAIN = None
CSRF_COOKIE_DOMAIN = None

# Avoid SameSite=None on http (Chrome may drop it)
